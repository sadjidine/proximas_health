...
class PriseEnCharge(models.Model):
    ...
    pathologie_ids = fields.Many2many(
        ...
        string="Affections",
        help='Veuillez Indiquer les affections concernées.',
        required=False,
    )
    ...
    details_pec_soins_crs_ids = fields.One2many (
        comodel_name="proximas.details.pec",
        inverse_name="pec_id",
        domain=['|', ('prestation_demande_id', '!=', None), ('prestation_crs_id', '!=', None),],
        # ondelete='cascade',
        string='Exécution Prestation Médicale',
    )
...
class DetailsPec(models.Model):
    ...
    def _stuff_note(self):
        ...
        stuff_to_notes = u"Le Centre d'orientation (CRS) : %s est en attente de la validation \
    ...
    def need_action_validation(self, cr, uid, context):
        if not (self.prestation_demande_id) and not (self.date_demande):
            ...
            # post_vars = {'subject': u"Demande Validation Prestations CRS",
            #              'body': u"Le Centre d'orientation (CRS) : %s est en attente de la validation \
            #               de la prestation suivante : \n Date Exécution:%s\n \
            #               Prestation médicale: %s \n Code PEC: %s \n Code ID.: %s \n Assuré: %s \n Genre: %s \n \
            #               Statut familial: %s."
...
class ReportSinistreRecapWizard(models.TransientModel):
    ...
    type_recap = fields.Selection(
        string="Régrouper par",
        selection=[
            ('rubrique', 'Rubrique Médicale'),
            ('prestation', 'Prestation Médicale'),
            ('police', 'Police (Produit Couverture)'),
    ...
    def get_report(self):
        ...
        return self.env['report'].get_action(self, 'proximas_medical.sinistre_recap_report_view', data=data)
...
class ReportPecDetailsRecap(models.AbstractModel):
    ...
    """
    _name = 'report.proximas_medical.sinistre_recap_report_view'

    @api.multi
    def render_html(self, data=None):
        report_obj = self.env['report']
        # print'>>>>>>>>>>>>>>>>>......', report_obj
        report = report_obj._get_report_from_name('proximas_medical.sinistre_recap_report_view')
        # print'>>>>>>>>>>>>>>>>>......', report
        ...
        docargs = {
            ...
            'date_debut': datetime.strftime(date_debut_obj, '%d-%m-%Y'),  # date_debut_obj.strftime(DATETIME_FORMAT),
            'date_fin': datetime.strftime(date_fin_obj, '%d-%m-%Y'),  # date_fin_obj.strftime(DATETIME_FORMAT),
            'docs': docs,
        }
        return report_obj.render('proximas_medical.sinistre_recap_report_view', docargs)
