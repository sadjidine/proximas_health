                    ...
                    if 0 < plafond < cout_unitaire and code_non_controle:
                        ...
                        if rec.mt_rabais > 0:
                            ...
                            rec.total_pc = int (
                                cout_unitaire * rec.coefficient * quantite)  # - int(rec.mt_exclusion)
                            rec.total_npc = rec.cout_total - rec.total_pc
                # Forfait général - Coût forfaitaire (sans tenir compte du coefficient
                # elif 0 < int(rec.forfait_general) and int(rec.forfait_general) <= int(rec.cout_unite):
                #     forfait = int(rec.forfait_general)
                #     # Sinon, si le coût de la prestation n'est pas modifiable, il y a t-il un coût forfaitaire
                #     if rec.mt_rabais:
