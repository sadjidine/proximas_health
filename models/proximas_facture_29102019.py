...
class Facture(models.Model):
    ...
    @api.one
    @api.depends ('details_pec_ids','num_facture')
    def _compute_facture_details(self):
        for rec in self:
            if bool(rec.details_pec_ids):
                ...
                rec.net_prestataire_facture = sum(item.net_prestataire for item in details_actes_facture) or 0

    @api.depends('mt_total_facture','num_facture')
    def montant_en_text(self):
        convert_lettre = amount_to_text_fr(self.mt_total_facture, 'Francs')
    ...
    def _get_num_facture(self):
        ...
        for rec in self:
            date_emission = fields.Datetime.from_string(rec.date_emission)
